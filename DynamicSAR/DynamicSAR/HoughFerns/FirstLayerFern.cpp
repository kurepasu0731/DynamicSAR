 ...
 * @param   poseIl[in]		姿勢クラスの偏りに応じた重み付け
 */
void FirstLayerFern::Learn(const std::vector<std::shared_ptr<LearnSample>>& samples, int posNum, const std::vector<double>& il, const std::vector<double> &poseIl)
{
	// 初期化
	splitNode.clear();
	nodeData.clear();
		...
		for (int f = 0; f < featureTestNum; ++f)
		{
			...
			for (int t = 0; t < thresholdTestNum; ++t)
			{
				...
				for (int n = 0; n < node_num; ++n)
				{
					...
					if (gain_switch){
						// 情報利得の算出
						gain += splitMultiInformationGain(samples, sample_indices, similarity, leftCount, rightCount, threshold, il, poseIl);
					} else {
						// オフセットの分散の算出
						gain += splitMultiVariance(samples, sample_indices, similarity, leftCount, rightCount, threshold);
					}
				}
	...
	for (int n = 0; n < node_num; ++n)
	{
		...
		// 姿勢確率密度の計算
		std::vector<double> poseDistribution( 8, 0.0);
		...
		for (int i = 0; i < sample_indices.size(); ++i)
		{
			...
			if (samples[index]->label == posLabel){
				int label = samples[index]->poseLabel;
				poseDistribution[label] += 1.0 * poseIl[label];
			}
		}
		// 正規化
		for (int i = 0; i < 8; ++i) {
			poseTotal +=poseDistribution[i];
		}
		for (int i = 0; i < 8; ++i){
			if (poseTotal != 0.0) {
				poseDistribution[i] /= poseTotal;
			} else {
				poseDistribution[i] = 0.0;
			}
		}
		...
		node_data->poseDistribution.assign( poseDistribution.begin(), poseDistribution.end());
...
// 識別と投票処理
void FirstLayerFern::Predict(const cv::Mat& src, cv::Mat_<float>& likelihoodMap, std::vector<cv::Mat_<float>> &poseMap)
{
	// 学習時間の測定
		...
		for (int c = 0; c < findLUT.cols; ++c)
		{
			...
			if (nodeData[index]->distribution[posLabel] > 0.65)
			{
				...
				for (int i = 0; i < offset_num; ++i)
				{
					...
					for (int j = 0; j < 8; ++j)
					{
						poseMap[j](y,x) += nodeData[index]->poseDistribution[j]*vote;
					}
				}
			}
		}
 ...
 * @returns		分岐後の情報利得
 */
inline double FirstLayerFern::splitMultiInformationGain( const std::vector<std::shared_ptr<LearnSample>> &samples, const std::vector<int> &indices, const std::vector<float> &similarity, 
												int &leftCount, int &rightCount, float threshold, const std::vector<double> &il, const std::vector<double> &poseIl)
{
	std::vector<double> leftDistribution(numClass, 0.0);	// 左のクラス確率
	std::vector<double> rightDistribution(numClass, 0.0);	// 右のクラス確率
	std::vector<double> leftPoseDistribution(8, 0.0);		// 左の姿勢クラス確率
	std::vector<double> rightPoseDistribution(8, 0.0);	// 右の姿勢クラス確率
	...
	for (int i = 0; i < (int)indices.size(); ++i)
	{
		...
		if (similarity[index] < threshold) {
			...
			if (samples[index]->label == posLabel) {
				leftPoseDistribution[ samples[index]->poseLabel ] += 1.0 * poseIl[ samples[index]->poseLabel ];
			}
		} else {
			rightCount++;
			rightDistribution[samples[index]->label] += 1.0 * il[samples[index]->label];
			if (samples[index]->label == posLabel) {
				rightPoseDistribution[ samples[index]->poseLabel ] += 1.0 * poseIl[ samples[index]->poseLabel ];
			}
		}
	}
	...
	else
	{
		...
		// 子ノードのエントロピーを計算
		double classEntropy = lsize / (lsize + rsize) * computeEntropy(leftDistribution) + rsize / (rsize + lsize) * computeEntropy(rightDistribution);


		///// 姿勢に関する情報利得 /////
		double poseEntropy = 0.0;

		leftTotal = 0.0, rightTotal = 0.0;

		// 分岐後の姿勢クラス確率の総和
		for (int i = 0; i < 8; ++i)
		{
			leftTotal += leftPoseDistribution[i]; 
			rightTotal += rightPoseDistribution[i]; 
		}
		// 正規化
		for (int i = 0; i < 8; ++i)
		{
			if (leftTotal != 0.0) {
				leftPoseDistribution[i] /= leftTotal;
			}
			if (rightTotal != 0) {
				rightPoseDistribution[i] /= rightTotal;
			}
		}

		// 子ノードのエントロピーを計算(姿勢)　ポジティブ確率*log2(分割数)*エントロピー
		poseEntropy = lsize / (lsize + rsize) * leftDistribution[posLabel] * computePoseEntropy(leftPoseDistribution)
					+ rsize / (rsize + lsize) * rightDistribution[posLabel] * computePoseEntropy(rightPoseDistribution);

		// 対象物体かどうかの重みの方を大きくする
		return classEntropy + poseEntropy;
	}
}
 ...
 /**
 * @brief   分岐後のオフセットの分散結果(姿勢のクラスも考慮)
 * 
 * @param	samples[in]				学習データ
 * @param	indices[in]				現在のノードにおけるサンプルのインデックス
 * @param	similarity[in]			類似度
 ...
 * @returns		分岐後の分散
 */
inline double FirstLayerFern::splitMultiVariance( const std::vector<std::shared_ptr<LearnSample>> &samples, const std::vector<int> &indices, const std::vector<float> &similarity, int &leftCount, int &rightCount, double threshold)
{
	std::vector<std::vector<cv::Point2f>> leftOffsetList(8);				// 左のオフセットベクトルのリスト
	std::vector<std::vector<cv::Point2f>> rightOffsetList(8);				// 右のオフセットベクトルのリスト
	...
	for (int i = 0; i < (int)indices.size(); ++i)
	{
		...
		if (similarity[index] < threshold) 
		{
			leftCount++;
			if (samples[index]->label == posLabel){
				leftOffsetList[samples[index]->poseLabel].push_back(samples[index]->offset);
			}			
		} else 
		{
			rightCount++;
			if (samples[index]->label == posLabel){
				rightOffsetList[samples[index]->poseLabel].push_back(samples[index]->offset);
			}
		}
	}
	...
	else
	{
		...
		for (int i = 0; i < 8; ++i)
		{
			...
			if (leftOffsetList[i].size() == 0){
				leftCentroid.x = 0.0f;
				leftCentroid.y = 0.0f;
			}else{
				// 左のサンプルのセントロイドの算出
				for (int j = 0; j < (int)leftOffsetList[i].size(); ++j)
				{
					leftCentroid.x += leftOffsetList[i][j].x;
					leftCentroid.y += leftOffsetList[i][j].y;
				}
				leftCentroid.x /= (float)leftOffsetList[i].size();
				leftCentroid.y /= (float)leftOffsetList[i].size();
			}
	
			if (rightOffsetList[i].size() == 0){
				rightCentroid.x = 0.0f;
				rightCentroid.y = 0.0f;
			}else{
				// 右のサンプルのセントロイドの算出
				for (int j = 0; j < (int)rightOffsetList[i].size(); ++j)
				{
					rightCentroid.x += rightOffsetList[i][j].x;
					rightCentroid.y += rightOffsetList[i][j].y;
				}
				rightCentroid.x /= (float)rightOffsetList[i].size();
				rightCentroid.y /= (float)rightOffsetList[i].size();
			}

			// 左の分散の算出
			for (int j = 0; j < (int)leftOffsetList[i].size(); ++j)
			{
				tmpX = leftCentroid.x - leftOffsetList[i][j].x;
				tmpX = tmpX * tmpX;
				tmpY = leftCentroid.y - leftOffsetList[i][j].y;
				tmpY = tmpY * tmpY;
			...
			if (leftOffsetList[i].size() == 0){
				...
				leftVariance /= (float)leftOffsetList[i].size();
			}

			//右の分散の算出
			for (int j = 0; j < (int)rightOffsetList[i].size(); ++j)
			{
				tmpX = rightCentroid.x - rightOffsetList[i][j].x;
				tmpX = tmpX * tmpX;
				tmpY = rightCentroid.y - rightOffsetList[i][j].y;
				tmpY = tmpY * tmpY;
			...
			if (rightOffsetList[i].size() == 0){
				...
				rightVariance /= (float)rightOffsetList[i].size();
			}

			// 子ノードの分散の和を返す
			totalVariance += (double)(leftVariance + rightVariance);
		}
 ...
 * @returns シャノンエントロピー   
 */
inline double FirstLayerFern::computeEntropy(const std::vector<double> &dist)
{
	double entropy = 0.0;
...
/**
 * @brief   シャノンエントロピー関数(姿勢)
 * 
 * @param   dist[in] 確率密度関数
 * 
 * @returns シャノンエントロピー   
 */
inline double FirstLayerFern::computePoseEntropy(const std::vector<double> &dist)
{
	double entropy = 0.0;
 ...
 */
void FirstLayerFern::saveFern(const std::string &fileName, const std::string& fileName_txt)
{
	// ファイルオープン
	...
	if (splitNode.size() > 0) {
		...
		ofs_t << splitNode[0]->patch.cols << std::endl;		// パッチサイズ
	} else {
		int p_size = 0;
		ofs.write((const char*)&p_size, sizeof(int));		// パッチサイズ
	...
	for (int i = 0; i < (int)splitNode.size(); ++i)
	{
		...
		// パッチの重み
		for (int j = 0; j < (int)weight_vec.size()-1; ++j)
		{
	...
	for (int i = 0; i < (int)nodeData.size(); ++i)
	{
		...
		if (nodeData[i]->offsetList.size() > 0)
		{
			...
			// 姿勢確率
			int pose_distribute_size = nodeData[i]->poseDistribution.size();
			ofs.write((const char*)&pose_distribute_size, sizeof(int));

			for (int j = 0; j < pose_distribute_size-1; ++j)
			{
				float p_dist = nodeData[i]->poseDistribution[j];
				ofs.write((const char*)&p_dist, sizeof(float));
				ofs_t << p_dist << ",";
			}
			float p_dist = nodeData[i]->poseDistribution[pose_distribute_size-1];
			ofs.write((const char*)&p_dist, sizeof(float));
			ofs_t << p_dist << std::endl;
 ...
 */
bool FirstLayerFern::loadFern(const std::string &fileName)
{
	// 初期化
	splitNode.clear();
	nodeData.clear();
	...
	for (int i = 0; i < valid_num; ++i)
	{
		...
		// 姿勢確率
		int pose_distribute_size;
		ifs.read((char*)&pose_distribute_size, sizeof(int));

		for (int j = 0; j < pose_distribute_size; ++j)
		{
			float p_dist;
			ifs.read((char*)&p_dist, sizeof(float));
			data->poseDistribution.emplace_back(p_dist);
		}
 ...
 * @returns true or false  
 */
bool FirstLayerFern::loadFern_txt(const std::string &fileName)
{
	// 初期化
	splitNode.clear();
	nodeData.clear();
	...
	for (int i = 0; i < valid_num; ++i)
	{
		...
		for (int j = 0; j < (int)params[count_num].size(); ++j)
		{
			data->poseDistribution.emplace_back(atof(params[count_num][j].c_str()));
		}
		count_num++;
